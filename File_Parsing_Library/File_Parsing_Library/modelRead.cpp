...
Layer *readConvLayer(std::ifstream &file) {
  ...
  tempLayer->setBias(readMatrix(file,
                                std::vector<int>{1, tempLayer->getFilters()},
                                tempLayer->getDataType()));
  ...
  for (int j = 0;
       j < tempLayer->getInputShape().at(tempLayer->getInputShape().size() - 1);
       j++)
    for (int i = 0; i < tempLayer->getFilters(); i++) {
      tempLayer->addKernel(readMatrix(file, tempLayer->getKernelDimensions(),
                                      tempLayer->getDataType()));
    }
...
Layer *readDenseLayer(std::ifstream &file) {
  ...
  // Bias data(Size based on type)
  tempLayer->setBiases(
      readMatrix(file, std::vector<int>{1, tempLayer->getOutputShape().at(0)},
                 tempLayer->getDataType()));
  ...
  tempLayer->setWeights(
      readMatrix(file,
                 std::vector<int>{tempLayer->getInputShape().at(0),
                                  tempLayer->getOutputShape().at(0)},
                 tempLayer->getDataType()));
...
MatrixXfRM *readMatrix(std::ifstream &file, std::vector<int> dimensions,
                       int dataType) {
  ...
  for (int i = 0; i < dimensions[0]; i++) {
    for (int j = 0; j < dimensions[1]; j++) {
      (*matrix)(i, j) = readDecimal(file, dataType == FLOAT32 ? sizeof(float)
                                                              : sizeof(double));
    }
  }
