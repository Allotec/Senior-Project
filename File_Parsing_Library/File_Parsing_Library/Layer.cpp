...
//Constructor
Layer::Layer(std::string name, int layerType, int dataType, std::vector<int> inputShape, std::vector<int> outputShape) {
	this->name = name;
...
uint8_t Layer::getLayerType() {
	return this->layerType;
}

uint8_t Layer::getDataType() {
...
void Layer::setDataType(uint8_t dataType) {
	this->dataType = dataType;
}
...
void Layer::print() {
	...
	std::cout << "Data Type: " << (this->dataType == 0 ? "Float32" : this->dataType == 1 ? "Float64" : "Uknown") << std::endl;
