...
// Constructors
PoolLayer::PoolLayer(std::vector<int> poolDimensions, std::vector<int> strides,
                     int padding, int poolType, std::string name, int dataType,
                     std::vector<int> inputShape, std::vector<int> outputShape)
    : Layer(name, poolType == 0 ? MAXPOOL_LAYER : AVGPOOL_LAYER, dataType,
...
// Getters
std::vector<int> PoolLayer::getPoolDimensions() { return this->poolDimensions; }

std::vector<int> PoolLayer::getStrides() { return this->strides; }

int PoolLayer::getPadding() { return this->padding; }

int PoolLayer::getPoolType() { return this->poolType; }
...
void PoolLayer::setPadding(int padding) { this->padding = padding; }
...
bool PoolLayer::calculateOutput() {
  ...
  for (int i = 0; i < this->inputMatrix->size(); i++) {
    this->outputMatrix->push_back(pooling(this->inputMatrix->at(i),
                                          this->padding, this->poolDimensions,
                                          this->poolType, this->strides));
  }
