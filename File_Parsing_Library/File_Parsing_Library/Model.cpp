...
Model::Model() { this->layers = std::vector<Layer *>(); }
...
void Model::addLayer(Layer *layer) { this->layers.push_back(layer); }
...
bool Model::calculateOutput(std::vector<MatrixXfRM> *input, int index) {
  ...
  int maxIndex = 0;
  for (int i = 0;
       i < this->layers[this->layers.size() - 1]->getOutputShape().at(0); i++) {
    if (this->layers[this->layers.size() - 1]->getOutputMatrix()->at(0)(0, i) >
        this->layers[this->layers.size() - 1]->getOutputMatrix()->at(0)(
            0, maxIndex)) {
      maxIndex = i;
    }
  }
...
void Model::outputModel(std::string path) {
  ...
  // Puts the input matrices into one flattened version and outputs it to a
  // binary file
  for (int i = 0; i < this->layers.size(); i++) {
