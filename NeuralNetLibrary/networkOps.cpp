...
MatrixXfRM convolution(MatrixXfRM input, int padding, MatrixXfRM kernel,
  ...
  for (int outputRows = 0, x = 0;
       outputRows < output.rows() && x < input.rows();
       outputRows++, x += stride.at(0)) {
    for (int outputCols = 0, y = 0;
         outputCols < output.cols() && y < input.cols();
         outputCols++, y += stride.at(1)) {
      // Weighted sum
...
std::vector<MatrixXfRM> *conv2d(std::vector<MatrixXfRM> input,
  ...
  for (int oc = 0; oc < num_output_channels; oc++) {
    ...
    for (int ic = 0; ic < num_input_channels; ic++) {
      for (int i = 0; i < output_rows; i++) {
        for (int j = 0; j < output_cols; j++) {
          output->at(oc)(i, j) +=
              (input.at(ic)
                   .block(i * stride, j * stride, filter_rows, filter_cols)
                   .cwiseProduct(*filter.at(filter_index)))
                  .sum();
        }
      }
      filter_index++;
    ...
    output->at(oc) += MatrixXfRM::Constant(output->at(oc).rows(),
                                           output->at(oc).cols(), bias(0, oc));
  }
...
MatrixXfRM pooling(MatrixXfRM input, int padding, std::vector<int> kernel,
                   int poolingType, std::vector<int> stride) {
...
MatrixXfRM dense(MatrixXfRM input, MatrixXfRM weights, MatrixXfRM bias,
                 int activation) {
  if (activation == SOFTMAX) {
    ...
    std::cout << "Error: Invalid Activation Function using default!"
              << std::endl;
  }
